...
Next.js 15 (App Router) 기반, Termux(Android/aarch64) 환경에서 실행 가능하도록 puppeteer 없이 브라우저 `window.print()` API로 PDF를 생성한다.
...
클릭할 때마다 `B6 → A5 → MA5 → B6` 순으로 페이지 크기를 순환한다.
...
public/
  fonts/
    bold/      ← 제목·강조체 후보 (ttf/otf/woff/woff2)
    regular/   ← 흐림체 후보
```

- `bold/` 와 `regular/` 양쪽 모두 배경체(bg) 폰트 선택지에 포함된다.
